...
\title{Parametric Tensor Network Bases for Sparse Image Representation via Riemannian Optimization}
...
% ============================================================================
\begin{abstract}
We introduce a framework for learning data-adaptive unitary transforms parameterized by quantum circuit structures for image compression.
The circuit parameters---Hadamard gates on the unitary group $U(2)$ and controlled-phase gates on the phase manifold $U(1)^4$---are optimized via Riemannian gradient descent and Riemannian Adam, preserving unitarity exactly at every iterate.
We systematically compare four tensor network topologies of increasing connectivity: the separable Quantum Fourier Transform (QFT), an Entangled QFT with cross-dimensional coupling, Time-Evolving Block Decimation (TEBD) with ring topology, and a MERA-inspired hierarchical circuit.
On standard benchmarks (Quick Draw at $32 \times 32$ and DIV2K at $256 \times 256$), we find that the learned bases match or exceed classical FFT baselines, with the Entangled QFT achieving the best overall performance.
Our analysis reveals that \emph{circuit connectivity topology} is the dominant factor determining compression quality: the Entangled QFT's cross-dimensional gates capture spatial correlations that separable architectures miss, while TEBD's local ring topology is insufficient for the non-local frequency structure of natural images.
We provide a production-grade open-source Julia implementation with GPU support, enabling reproducible benchmarking of parametric quantum circuit bases.
\end{abstract}
...
\begin{enumerate}
  \item A systematic framework for parametric tensor network bases, unifying QFT, Entangled QFT, TEBD, and MERA-inspired circuits under a common Riemannian optimization scheme on products of $U(2)$ and $U(1)^4$ manifolds.
  \item Empirical comparison of four circuit topologies on standard image compression benchmarks, establishing that learned quantum circuit bases are competitive with classical FFT and that circuit connectivity topology is the primary determinant of compression quality.
  \item A frequency-weighted top-$k$ truncation scheme with a custom straight-through gradient estimator that enables end-to-end differentiable training of the reconstruction loss.
  \item An open-source Julia library (\texttt{ParametricDFT.jl}) with batched einsum contractions, Riemannian optimizers, and optional GPU acceleration.
\end{enumerate}
...
Sparse representation of signals is fundamental to data compression: given a transform basis $\TT$, a signal $\mathbf{x}$ is compressed by retaining only the $k$ largest coefficients of $\mathbf{y} = \TT(\mathbf{x})$ and discarding the rest.
The quality of the compressed representation depends critically on how well the basis concentrates signal energy into a small number of coefficients.

Classical transforms such as the Discrete Fourier Transform (DFT) and the Discrete Cosine Transform (DCT) provide fixed bases with well-understood sparsity properties for periodic and smooth signals~\cite{ahmed1974dct, cooley1965fft}.
However, these bases are not adaptive to the statistics of specific signal classes: the DFT assumes periodic boundary conditions and separable spatial structure, neither of which holds for natural images with edges, textures, and diagonal features.

\paragraph{Tensor network perspective.}
The Cooley--Tukey FFT algorithm~\cite{cooley1965fft} admits a natural decomposition as a tensor network of two-qubit gates: Hadamard gates $H$ and controlled-phase gates $M_k = \diag(1, 1, 1, e^{i\pi/2^{k-1}})$.
With fixed gate parameters, this tensor network computes the DFT exactly.
A key observation motivating this work is that the gate parameters can be \emph{relaxed}: by allowing $H$ to range over the full unitary group $U(2)$ and the phase entries of $M_k$ to become learnable, we obtain a family of \emph{parametric} unitary transforms that includes the DFT as a special case.
The optimization then seeks the point on this parameter manifold that yields the sparsest representation for a given dataset.

\paragraph{Riemannian optimization.}
The circuit parameters live on Riemannian manifolds: Hadamard gates on $U(2)$ and controlled-phase gates on the product of unit circles $U(1)^4$.
Na\"ive Euclidean gradient updates would violate the unitarity constraint, requiring expensive re-orthogonalization or penalty terms.
We instead employ Riemannian gradient descent with Armijo backtracking and Riemannian Adam~\cite{becigneul2019riemannian}, which project gradients onto the tangent space and retract updates back to the manifold, preserving unitarity exactly at every iterate.

\paragraph{Circuit topology matters.}
Beyond the basic QFT structure, we explore three additional topologies that introduce different connectivity patterns between qubits.
The \emph{Entangled QFT} adds controlled-phase gates coupling row and column qubits, capturing cross-dimensional correlations.
\emph{TEBD}~\cite{vidal2004tebd} arranges gates in a nearest-neighbor ring topology.
A \emph{MERA-inspired}~\cite{vidal2007mera} circuit uses hierarchical multi-scale connectivity.
Our experiments reveal that topology is the dominant factor: the Entangled QFT, despite adding only $n$ extra phase parameters, consistently outperforms both the separable QFT and the more complex TEBD topology.
...
Parameterized quantum circuits have been studied for variational quantum algorithms~\cite{cerezo2021variational, mcclean2016variational} and quantum machine learning~\cite{schuld2019quantum, benedetti2019parameterized}.
The use of Riemannian optimization for unitary constraints appears in quantum control~\cite{khaneja2005optimal} and manifold learning~\cite{absil2008optimization}.
Our work connects these threads by using quantum circuit structures as \emph{classical} tensor networks for image compression, optimized on their natural parameter manifolds.
...
\begin{itemize}
  \item \textbf{Hadamard gates}: $H = \begin{psmallmatrix} 1 & 1 \\ 1 & -1 \end{psmallmatrix}$, one per qubit.
...
On Quick Draw (\cref{tab:quickdraw}), the Entangled QFT achieves the best performance across all compression ratios, improving over the classical FFT by up to $+0.20$~dB in PSNR and $+0.015$ in SSIM at 20\% compression.
The separable QFT shows modest gains at aggressive compression (5\%) but converges to FFT performance at higher ratios, suggesting that the learned gates recover parameters close to the DFT values.
TEBD consistently underperforms, with a $-0.92$~dB deficit at 10\% compression compared to FFT, indicating that the ring topology's local connectivity is insufficient for the frequency structure of even simple sketch images.
...
On DIV2K (\cref{tab:div2k}), all learned bases improve over the FFT baseline, with the Entangled QFT and QFT performing nearly identically.
The marginal gain of entanglement is smaller on high-resolution images ($+0.01$~dB), likely because the 8-qubit circuits have more parameters in the QFT structure itself, reducing the relative impact of the $n = 8$ additional entanglement phases.
TEBD again lags significantly ($-1.51$~dB at 10\%), confirming that the ring topology is systematically suboptimal for 2D image compression.
...
The Entangled QFT converges faster than the QFT ($127$~s vs.\ $291$~s on DIV2K), despite having more parameters.
This suggests that the entanglement gates provide useful cross-dimensional gradients that accelerate optimization.
TEBD converges to a significantly higher training loss ($1752.6$ vs.\ $1162.0$), reflecting its limited expressiveness.
Interestingly, TEBD's validation loss is lower than its training loss, suggesting that the ring topology acts as an implicit regularizer---but at the cost of underfitting.
...
\paragraph{Why learned bases match but don't dramatically exceed classical FFT.}
The DFT is provably optimal for periodic signals and near-optimal for smooth functions~\cite{donoho2006compressed}.
Natural images are not periodic, but the DFT's uniform coverage of the frequency plane is difficult to beat without fundamentally changing the transform structure (as wavelets do by trading frequency resolution for spatial resolution).
Our parametric bases explore a neighborhood of the DFT on the parameter manifold; the learned optima lie close to the DFT parameters, confirming that the FFT is already a strong local minimum for general images.
The gains come from fine-tuning the frequency responses to specific image statistics.

\paragraph{The value of cross-dimensional entanglement.}
The Entangled QFT's consistent advantage, despite minimal parameter overhead, supports the hypothesis that cross-dimensional correlations in natural images are not fully captured by separable transforms.
The entanglement gates allow the basis to represent frequency components that mix row and column information---analogous to diagonal Gabor-like features that a separable basis cannot express.

\paragraph{Limitations and future work.}
Our current benchmarks compare only against the classical FFT.
A comprehensive evaluation should include the DCT (JPEG's basis), wavelet transforms (JPEG2000), and learned dictionaries (K-SVD).
We expect that the parametric bases may show larger advantages on specific image classes (e.g., structured/self-similar images) or at extreme compression ratios.
Additionally, ablation studies comparing Riemannian vs.\ projected Euclidean optimization would quantify the contribution of the manifold-aware training scheme.
The MERA-inspired basis requires power-of-two qubit counts per dimension and was not benchmarked on the two primary datasets; evaluating it on $16 \times 16$ or $64 \times 64$ patches is a direction for future work.
...
We have presented a systematic framework for learning sparse image representations using parametric quantum circuit structures optimized on Riemannian manifolds.
By comparing four tensor network topologies---QFT, Entangled QFT, TEBD, and MERA-inspired circuits---we have shown that circuit connectivity topology is the primary determinant of compression quality.
The Entangled QFT, which introduces cross-dimensional coupling between row and column qubits, achieves the best performance with minimal parameter overhead.
Riemannian optimization on the natural parameter manifolds ($U(2)$ for Hadamard gates, $U(1)^4$ for phase gates) preserves unitarity exactly and enables efficient training.
While the learned bases are competitive with classical FFT rather than dramatically superior, they establish a principled, extensible framework for data-adaptive unitary transforms grounded in quantum circuit structures.
